...
import {
  ...
  InfoPagina,
  CardItems
} from "../styles/ListarPesq";
import * as S from '../styles/InfoPesq'

function PesqsSolicitadas() {
  ...
  function carregarPagina(numeroDaPagina) {
    ...
    fetch(`${API_URL}/pesq_solicitadas/?page=${numeroDaPagina}&status=${statusAtual}`, {
      method: 'GET',
      headers: {
          ...
          {dados.map((item) => (
            <CardItems key={item.id_public}>
              ...
              <div style={{ marginTop: '12px', display: 'flex', justifyContent: 'flex-end' }}>
                <a
                  onClick={() => infoPesquisa(item.id_public)}
                  style={{
                    textDecoration: 'none',
                    color: '#7c3aed',
                    fontWeight: 600,
...
export default PesqsSolicitadas;
